...
def quantize_with_hessian_per_row(
    ...
    # Act-order step:
    # sort columns by importance before GPTQ-style processing, then restore
    # the original column order at the end so the saved weights keep the
    # normal layout expected by the model.
    if column_order is not None:
        W_quant = W_quant.index_select(1, column_order)
        H_inv = H_inv.index_select(0, column_order).index_select(1, column_order)
    ...
    if column_order is not None:
        ...
        inverse_order[column_order] = torch.arange(n_in, device=column_order.device)
        W_quant = W_quant.index_select(1, inverse_order)
...
if __name__ == "__main__":
    ...
    OUTPUT_PATH = os.getenv("OUTPUT_PATH", "checkpoints/mistral_7b_instruct_v03_gptq_nf4_optimized_CC_1_BC_128")
    ...
    CHUNK_COLS = int(os.getenv("CHUNK_COLS", "1"))
    ...
    print(f"Act-order enabled: {ACT_ORDER}")
    ...
    for block_index in block_pbar:
        ...
        for layer_name in layer_pbar:
            ...
            with torch.no_grad():
                ...
                column_order = None
                if ACT_ORDER:
                    # Rank columns by Hessian diagonal importance so the most
                    # important columns are quantized earlier.
                    column_order = torch.argsort(torch.diag(H), descending=True)
